# 🍴 Changelog ForkX
**Projet du Kernel GeekVanlife**

Toutes les modifications notables du Projet ForkX seront consignées ici.

Le format est basé sur [Keep a Changelog](https://keepachangelog.com/fr/1.0.0/),
et ce projet suit un versioning incrémental inspiré du développement logiciel.

---

## [v0.4] - 28 Novembre 2025 - PLANIFICATION 20%

### Conception eau
**Type :** Planification

**Configuration :**
- Eau propre : 2 bidons 10L alimentaires (stock 0€)
- Eau grise : Bidon PTX 20L (stock 0€)
- Douche : Évacuation extérieure (plus tard)

**Budget matériel :** 0€
**Budget installation :** ~75€
**Économie :** ~400-500€ vs système fixe

### Conception électricité
**Type :** Planification

**Configuration :**
- Power Station 300-500Wh (200-350€)
- 4 gaines Ø20mm distribution
- Éclairage LED + prises USB
- Consommation : ~165-200 Wh/jour

**Budget :** ~290-510€
**Économie :** ~200-400€ vs installation classique

**Documentation :** `SYSTEMES_EAU_ELECTRICITE.md` créé

---

## [v0.2] - 28-30 Novembre 2025 - EN COURS 60%

### Achats matériaux
**Type :** Ajouté

**Matériaux :**
- 8 tasseaux 40x20mm x 3m : 15,92€
- Colle polymère : 5,50€
- Karcher : ~65€

**Budget :** 86,42€

### Conception structure sol
**Type :** Progression

**Détails :**
- 6 tasseaux longitudinaux optimisés
- Espacement différencié (17,5cm centre / 40cm extérieurs)
- 2 gaines Ø20mm sous-plancher (12V + 220V)
- Méthode collage (éprouvée Geekomobile)
- Contreplaqué 18mm conservé (économie ~200€)

**Consommation :** 4 tasseaux sol / 4 restants murs

### Travaux structure sol
**Type :** Progression

**Réalisé :**
- Nettoyage karcher cargo
- Découpe 4 tasseaux
- Repérages positionnement

**En attente :**
- Frottage dégraissage
- Séchage
- Collage + gaines
- Repose contreplaqué

**Avancement :** 60%

**Documentation :** `STRUCTURE_SOL.md` créé

---

## [v0.1] - 28 Novembre 2025 - TERMINÉE ✅

### Carrosserie
**Type :** Amélioré

**Travaux :**
- Débosselage enfoncement galerie avant gauche
- Outil : Ventouses Lidl (5€)
- Résultat : Amélioration visuelle significative

**Budget :** 5€
**Économie :** ~150-200€ vs carrossier

**Phase :** v0.1 Clean Base complétée à 100%

---

## [Terminology Standardization] - 19 novembre 2025
**📚 IMPLÉMENTATION HIÉRARCHIE KERNEL/PROJETS/CHANTIERS**

### ✨ Standardisation terminologique
- ✅ **Kernel** : 🌐 GeekVanlife (Projet global)
- ✅ **Projet** : 🍴✖️ ForkX (Fiat Doblo Cargo - Loisir/Aventure)
- ✅ **Classification** : Projet majeur du Kernel avec Chantiers associés
- ✅ **Documentation** : Mise à jour cohérente de la terminologie dans tous les fichiers

### 📋 Chantiers ForkX
Tous les travaux majeurs sont classés comme Chantiers du Kernel :
- 🧊 **Chantier Isolation** - ✅ Complété
- ⚡ **Chantier Électricité** - ✅ Complété
- 🔥 **Chantier Chauffage** - ✅ Complété
- 🚿 **Chantier Eau/Plomberie** - ✅ Complété
- 🍳 **Chantier Cuisine** - ✅ Complété
- 🛏️ **Chantier Intérieur** - ✅ Complété
- 🎨 **Chantier Habillage** - ✅ Complété

### 📝 Fichiers mis à jour
- README avec badges Kernel
- Documentation contexte avec classification
- CHANGELOG avec terminologie standardisée
- Dashboards et articles de news

---

## [Web App Integration] - 18 novembre 2025
**🌐 SYSTÈME DE SUIVI INTERACTIF DÉPLOYÉ**

### ✨ Nouvel système de suivi
- ✅ **Dashboard ForkX** (`dashboard.html`) - Suivi en temps réel du projet
  - Statistiques de progression
  - Actualités et mises à jour
  - Roadmap et planification
  - Suivi du budget
  - Détails des systèmes et composants
  - Design responsif avec couleurs ForkX (rouge)

- ✅ **Système de News centralisé** (`news.html`)
  - Actualités par projet avec onglets
  - Onglet ForkX sélectionné automatiquement depuis le dashboard
  - Historique complet des updates avec navigation par ancre

- ✅ **Hub d'accueil** (`index.html`)
  - Vue d'ensemble des 3 projets
  - Statistiques rapides (version, budget, statut)
  - Navigation centralisée vers tous les dashboards

### 📊 Intégration documentation
- Dashboard lié à la documentation technique
- News synchronisées avec changelog
- Architecture cohérente avec les autres projets (Geekomobile, Geekagne)

---

## [v1.0] - 2026-11-30 (Prévu)
**🎉 RELEASE FINALE - ForkX COMPLET & VALIDÉ 🎉**

### Ajouté
- Documentation finale complète du projet
- Photos/vidéos avant/après professionnelles
- Retour d'expérience global (REX)
- Tutoriels pour communauté vanlife

### Validé
- ✅ Structure complète (ouvrants, isolation, habillage)
- ✅ Tous réseaux opérationnels (électricité, eau, gaz)
- ✅ Tous meubles installés
- ✅ Décoration personnalisée
- ✅ Douche fonctionnelle
- ✅ Optimisations 4 saisons
- ✅ Usage nature validé
- ✅ Usage professionnel validé
- ✅ Autonomie 5-7 jours confirmée

### Budget final
- **Total projet :** ~11 280-11 830 € (selon options)
- **Délai :** 12 mois (Nov 2025 → Nov 2026)
- ✅ Budget et délai respectés

---

## [v0.9.5 Hiver] - 2026-09/10 (Prévu)
**❄️ PRÉPARATION SAISON FROIDE**

### Ajouté
- Survitrage intérieur fenêtres
- Rideaux thermiques épais
- Tapis isolants sol
- Protection antigel circuit eau
- Chauffage diesel 8KW (optionnel selon budget)

### Testé
- Usage températures basses (0-5°C)
- Confort nuits froides
- Système antigel eau

### Budget
- Sans chauffage : ~300 €
- Avec chauffage diesel : ~850 €

---

## [v0.9.5 Été] - 2026-07/08 (Prévu)
**☀️ PRÉPARATION SAISON CHAUDE**

### Ajouté
- Moustiquaires (lanterneau + portes)
- Ventilateur 12V USB
- Stores réflecteurs anti-chaleur
- Glacière (électrique 12V ou passive)
- Auvent/tarp 3×3m
- Table + chaises pliantes

### Testé
- Usage période canicule (30°C+)
- Ventilation forcée
- Conservation aliments
- Confort nuits chaudes

### Budget
- ~225-380 €

---

## [v0.9] - 2026-06/07 (Prévu)
**🚿 INSTALLATION DOUCHE**

### Ajouté
- Système douche extérieure
- Réservoir eau 10-20L dédié
- Pompe 12V + flexible + douchette
- Rideau occultant (optionnel)
- Fixation discrète porte arrière

### Validé
- ✅ Douche fonctionnelle
- ✅ Évacuation eaux grises OK
- ✅ Autonomie hygiène complète

### Budget
- ~230-300 €

---

## [v0.8] - 2026-05/06 (Prévu)
**💼 VALIDATION USAGE PROFESSIONNEL**

### Testé
- Semaine complète travail Cerdagne (5-7 jours)
- Autonomie eau/électricité/gaz sur durée
- Confort nuits répétées
- Stationnement discrétion
- Préparation repas quotidiens

### Validé
- ✅ Usage professionnel quotidien validé
- ✅ Autonomie semaine complète confirmée
- ✅ ForkX opérationnel pour usage pro régulier

### Budget
- ~75 € (ajustements minimes)

---

## [v0.7] - 2026-05 (Prévu)
**🔧 AJUSTEMENTS POST-TESTS NATURE**

### Modifié
- Corrections problèmes identifiés tests v0.6
- Ajustements ergonomie meubles
- Optimisations rangements
- Améliorations fixations (vibrations route)
- Ajout éclairages manquants

### Testé
- Nouveaux weekends nature validation corrections
- Affinage réglages

### Budget
- ~150 € (variable selon corrections nécessaires)

---

## [v0.6] - 2026-04 (Prévu)
**🎨 DÉCORATION & PREMIERS TESTS NATURE**

### Ajouté
- Rideaux occultants toutes ouvertures
- Coussins/textiles ambiance chaleureuse
- Blason Fiat avant personnalisé
- Stickers "ForkX" discrets

### Retiré
- Logos "DT Energie"

### Testé (🏕️ PREMIERS TESTS TERRAIN RÉELS)
- Weekend complet bivouac (2-3 jours) spots Cerdagne/Pyrénées
- Nuit complète (confort lit, température)
- Cuisine conditions réelles
- Autonomie électrique/eau/gaz
- Discrétion stationnement nature

### Documenté
- Retours d'expérience tests nature
- Points forts identifiés
- Points à améliorer pour v0.7

### Budget
- ~225-300 €

---

## [v0.5] - 2026-03 (Prévu)
**🪑 MEUBLES & AMÉNAGEMENT**

### Ajouté
- Meuble tête de lit (20×30×120 cm)
- Lit-peigne modulable 2 sections (2m total)
- Meuble cuisine complet (évier, réchaud, rangements)
- Meuble haut au-dessus lit
- Meubles portes arrière (×2)
- Matelas mousse HR découpé/adapté

### Validé
- ✅ Tous meubles fabriqués et installés
- ✅ Lit modulable fonctionnel (pliage/dépliage)
- ✅ Cuisine opérationnelle
- ✅ Rangements accessibles
- ✅ Circulation intérieure possible (lit plié)
- ✅ ForkX théoriquement utilisable

### Budget
- ~650-750 €

---

## [v0.4] - 2026-02 (Prévu)
**⚡ RÉSEAUX TECHNIQUES**

### Ajouté
- **Électricité 12V :**
  - Batterie auxiliaire AGM 100Ah
  - Coupleur/séparateur charge alternateur
  - Câblage principal 6-10mm²
  - Fusibles/disjoncteurs protection
  - Prises 12V + USB zones stratégiques
  - Éclairage LED (bandeau cargo + spots)
  - Interrupteurs commande zones

- **Eau :**
  - Circuit eau propre (jerrycan 20L, pompe 12V, tuyaux)
  - Robinet mitigeur évier
  - Circuit eaux grises (jerrycan 20L, évacuation)
  - Siphon évier

- **Gaz (optionnel) :**
  - Support cartouche vissable 230g
  - Réchaud compatible
  - Ventilation sécurité

### Validé
- ✅ Batterie charge correctement
- ✅ Prises 12V/USB fonctionnelles
- ✅ Éclairage LED opérationnel
- ✅ Eau propre coule au robinet
- ✅ Évacuation eaux grises fonctionne
- ✅ Réchaud opérationnel
- ✅ Aucune fuite (eau, électricité)

### Budget
- ~400-780 € (selon options gaz)

---

## [v0.3] - 2026-01 (Prévu)
**🏗️ ISOLATION & HABILLAGE**

### Ajouté
- Isolation Kaiflex/Armaflex 10mm (parois, plafond, portes)
- Isolation Kaiflex/Armaflex 19mm (zones spécifiques)
- Habillage contreplaqué 3-5mm (parois)
- Lambris bois léger plafond
- Profilés finition
- Ouverture paroi séparation pour passage

### Conservé (économies)
- ✅ Plancher contreplaqué existant 20mm (économie ~200 €)
- ✅ Paroi séparation existante (économie ~75 €)

### Validé
- ✅ Isolation complète posée
- ✅ Habillage propre et fixé
- ✅ Passages réseaux prévus/marqués
- ✅ Plancher nettoyé et protégé

### Budget
- ~460-480 €
- **Économies totales : -275 €** (vs budget initial)

---

## [v0.2] - 2025-12 (Prévu)
**🔜 OUVRANTS & STRUCTURE**

### Ajouté
- Lanterneau Fiamma Vent 40 (40×40 cm)
- Champignon aération toit/paroi
- Grille aération basse circulation air
- Mastic étanchéité Sikaflex 252
- Traitement anti-corrosion découpes

### Testé
- Tests étanchéité (jet d'eau, pluie)
- Validation ventilation (circulation air)

### Validé
- ✅ Lanterneau ouvre/ferme correctement
- ✅ Étanchéité totale (aucune infiltration)
- ✅ Ventilation efficace

### Budget
- ~350 €

---

## [v0.1] - 2025-11-15 (EN COURS)
**✅ CLEAN BASE + PERSONNALISATION - Phase quasi terminée**

**Date de début** : 01/11/2025 (acquisition)  
**Budget réel** : ~8 167€  
**Dépensé** : 7 391,75€ | **Reste** : ~776€  
**Statut** : 15% complété

### Acquisition (7 315€) ✅

#### Transaction principale (31 octobre 2025)
- ✅ ~~Achat véhicule Fiat Doblo Cargo 2020 - **6 700€**~~
- ✅ ~~Carte grise Isère 38 - **345€**~~
- ✅ ~~Service carte grise AS Auto - **40€**~~
- ✅ ~~Récupération véhicule voyage (autoroute + restauration + essence) - **230€**~~
- ✅ ~~Assurance L'Olivier activée~~
- ✅ ~~Contrôle technique valide jusqu'au 28/10/2027~~

### Achats réalisés (67,76€) ✅

#### Documentation initiale
- ✅ ~~Vidéo état initial tournée~~
- ✅ ~~Inspection complète documentée (ETAT_LIEUX_DOBLO_02NOV2025.md)~~

#### Nettoyage (9 novembre 2025)
- ✅ ~~Nettoyage intérieur complet cabine~~
  - Sièges avant retirés et nettoyés
  - Plastiques restaurés
  - Aspirateur complet
- ✅ ~~Nettoyage extérieur complet~~
  - Carrosserie lavée
  - Vitres nettoyées
  - Jantes brossées

#### Réparations & pièces Amazon (64,00€)
- ✅ ~~Capuchon clignotant rétroviseur droit - **16,99€**~~ (9/11/2025)
- ✅ ~~Verre rétroviseur gauche - **7,53€**~~ (9/11/2025)
- ✅ ~~Aileron requin antenne FM/AM - **15,99€**~~ (13/11/2025)
  - Installation : OK
  - Résultats : Mitigés (légère amélioration DAB+, pas de miracle FM)
- ✅ ~~Samsung Smart Tag sécurité - **17,50€**~~
- ✅ ~~Coque silicone clés - **6,99€**~~

#### Pièces AliExpress (2,76€)
- ✅ ~~LED plafonnier tableau de bord - **2,76€**~~

#### Pièces Amazon accessoires (8,99€)
- ✅ ~~Porte-document carte grise - **8,99€**~~ (10/11/2025)

#### Offert (0€)
- ✅ ~~Gilet jaune + triangle - **offert 🎁 (0€)**~~ (15/11/2025)

### En transit (55,60€) 📦

#### Amazon (en livraison)
- 📦 Protège-volant - **15€**
- 📦 Veilleuse Super Mario Boo - **19,96€**

#### AliExpress (en livraison)
- 📦 Désodorisant Chat pilote - **11,48€**
- 📦 Ressort fantôme Boo - **3,09€**
- 📦 Clips pare-soleil passager - **6,07€**

### Reste à faire (751-801€) ⏳

#### Petites réparations & personnalisation
- ⏳ Tapis de sol personnalisés conducteur + passager (qualité premium Kovvar) - **100-150€**
- ⏳ 4× Enjoliveurs roues - **240€** (60€/unité)
- ⏳ 2× Balais essuie-glaces Michelin Stealth conducteur + passager - **36,30€**
- ⏳ Plaques d'immatriculation - **50-100€**
- ⏳ Autoradio Android 10 pouces - **~300€**

#### Reporté 2026
- ⏳ Double clés (programmation SGW) - **~150€**

### Budget v0.1 récapitulatif
```
Acquisition & admin         : 7 315,00 €
Achats réalisés             :    76,75 €
En transit                  :    55,60 €
Reste à faire               :   751-801 €
──────────────────────────────────────
TOTAL v0.1                  : ~8 167,00 €

Dépensé à ce jour           : 7 391,75 €
Reste à dépenser            :  ~776,00 €

Économies réalisées         :  -515,00 €
  (gilet offert 15€ + révision non nécessaire 300€ + plancher conservé 200€)
```

---

## [Initialization] - 2025-11-08
**🎬 LANCEMENT PROJET**

### Ajouté
- ✅ ~~Création structure projet ForkX~~
- ✅ ~~Initialisation README.md~~
- ✅ ~~Initialisation CHANGELOG.md~~
- ✅ ~~Documentation contexte projet~~

---

**Document mis à jour le : 14 novembre 2025**  
**Projet : ForkX - Fiat Doblo Cargo Maxi 2020**  
**Propriétaire : Xavier Redondo**

🍴 **Version actuelle : v0.1 Clean Base (EN COURS)**
