                ...
                for (let i = 0; i < localStorage.length; i++) { const key=localStorage.key(i); if
                    ...
                    */
                    ...
                    if (!trimmed || trimmed.length < 3) return; // Détecter si la tâche est complétée const
                        ...
                        // Valider que c'est un nombre valide pour amount
                        if (normalizedTodo.amount !== null && isNaN(normalizedTodo.amount)) {
                        normalizedTodo.amount = null;
                        }

                        mergedTodos.push(normalizedTodo);
                        importedCount++;
                        });

                        saveTodos(projectId, mergedTodos);
                        updateListSelect(projectId);
                        renderTodos(projectId);

                        let message = `Import réussi ! ${importedCount} tâche(s) importée(s).`;
                        if (skippedCount > 0) {
                        message += `\n${skippedCount} tâche(s) existante(s) ignorée(s).`;
                        }
                        alert(message);
                        ...
                        // Helpers pour hiérarchie parent/enfant
                        ...
                        if (completed) {
                        // Si on veut décoche une tâche, toujours possible
                        return true;
                        }

                        // Si on veut cocher une tâche parent, vérifier que tous les enfants sont cochés
                        const children = getTodoChildren(todos, todoId);
                        if (children.length === 0) {
                        // Pas d'enfants, toujours possible
                        return true;
                        }

                        // Vérifier que tous les enfants sont cochés
                        ...
                        // Fonction helper pour rendre une tâche et ses enfants
                        const renderTodoHierarchy = (todo, allTodos, depth = 0) => {
                        const children = getTodoChildren(allTodos, todo.id);
                        const hasChildren = children.length > 0;

                        return `
                        <div class="todo-item ${project.displayColor} ${todo.completed ? 'completed' : ''} todo-depth-${depth}"
                            draggable="true"
                            ...
                            ondragleave="leaveTodoDrag(event)" ondragend="endTodoDrag(event)"
                            onclick="toggleTodoExpand(event, '${projectId}', ${todo.id})" data-todo-id="${todo.id}"
                            data-list-name="${escapeHtml(todo.list)}">
                            <input type="checkbox" class="todo-checkbox" ${todo.completed ? 'checked' : '' }
                                onchange="toggleTodo('${projectId}', ${todo.id}); event.stopPropagation();">
                            <div class="todo-content">
                                <div class="todo-name">${escapeHtml(todo.name)} ${hasChildren ? `<span
                                        class="todo-children-count">(${children.length})</span>` : ''}</div>
                                <div class="todo-meta">
                                    <span class="priority-badge ${getPriorityClass(todo.priority)}"
                                        title="Priorité: ${todo.priority}"></span>
                                    ${todo.amount ? `<span class="todo-amount">${todo.amount.toFixed(2)}€</span>` : ''}
                                    ${hasChildren ? `
                                    <span class="todo-separator-inline"></span>
                                    <span class="todo-children-amount">
                                        <i class="fas fa-sitemap"></i> ${calculateChildrenAmount(allTodos,
                                        todo.id).toFixed(2)}€
                                    </span>
                                    ` : ''}
                                    ${todo.link ? `<a href="${escapeHtml(todo.link)}" target="_blank"
                                        class="todo-link-icon" title="${escapeHtml(todo.link)}"><i
                                            class="fas fa-link"></i></a>` : ''}
                                </div>
                                ${todo.note ? `<div class="todo-note-preview">${escapeHtml(todo.note)}</div>` : ''}
                            </div>
                            <div class="todo-actions" onclick="event.stopPropagation();">
                                <button class="action-icon action-icon-edit"
                                    onclick="openEditModal('${projectId}', ${todo.id})" title="Éditer"><i
                                        class="fas fa-edit"></i></button>
                                <button class="action-icon action-icon-delete"
                                    onclick="deleteTodo('${projectId}', ${todo.id})" title="Supprimer"><i
                                        class="fas fa-trash-alt"></i></button>
                            </div>
                        </div>
                        ${children.map(child => renderTodoHierarchy(child, allTodos, depth + 1)).join('')}
                        `;
                        };

                        listsContainer.innerHTML = Object.entries(todosByList).map(([listName, listTodos], index) => {
                        const estimate = calculateListEstimate(listTodos);
                        // Afficher seulement les tâches racines (sans parent)
                        ...
                        // Si on drop sur une autre tâche dans la même liste, demander confirmation pour créer relation
                        parent/enfant
                        ...
                        const newIndex = draggedIndex < targetIndex ? targetIndex - 1 : targetIndex;
                            todos.splice(newIndex, 0, movedTodo); saveTodos(projectId, todos); renderTodos(projectId); }
                            } } else if (draggedTodoList !==targetListName) { // Déplacement vers une liste différente
                            draggedTodo.list=targetListName; draggedTodo.parentId=null; // Réinitialiser le parent si
                            changement de liste saveTodos(projectId, todos); renderTodos(projectId); }
                            draggedTodoId=null; draggedTodoList=null; draggedProjectId=null; } function
